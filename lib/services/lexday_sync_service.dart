...
class ReadonSyncService {
  ...
  static String get _baseUrl => Env.lexdaySyncUrl;
  ...
  static Future<void> finishBook(int bookId) async {
    try {
      ...
      if (response.statusCode != 200) {
        debugPrint('lexday-sync finishBook error: ${response.body}');
      }
    } catch (e) {
      debugPrint('lexday-sync finishBook error: $e');
    }
  }
  ...
  static Future<void> renderMonthlyWrapped(int month, int year) async {
    try {
      ...
      if (response.statusCode != 200) {
        debugPrint('lexday-sync renderMonthlyWrapped error: ${response.body}');
      }
    } catch (e) {
      debugPrint('lexday-sync renderMonthlyWrapped error: $e');
    }
  }
  ...
  static Future<ShareAssets> getMonthlyWrappedShareAssets(
    ...
    try {
      ...
      debugPrint('lexday-sync getMonthlyWrappedShareAssets error: $e');
      return ShareAssets(status: 'none');
    }
  }
}
