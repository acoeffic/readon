-- Fix : get_user_notifications plantait avec
--   "invalid input syntax for type integer: 2.3139916..."
--
-- Cause : `duration_minutes` est stocké comme NUMERIC fractionnaire dans
-- activities.payload (ex. 2.3139916), et la version précédente de la RPC
-- castait directement avec `(payload->>'duration_minutes')::INT`. Quand un
-- seul utilisateur avait au moins une notif `like`/`comment` pointant sur
-- une activité avec duration fractionnaire, toute la liste de notifs
-- échouait — y compris les `group_join_request` n'utilisant pourtant pas
-- ce champ. Symptôme côté app : centre de messages vide, demandes
-- d'adhésion invisibles.
--
-- Fix : passer par NUMERIC avant INT (truncation propre), et faire pareil
-- pour start_page / end_page par sécurité.
    ...
    CASE
      WHEN n.type IN ('like', 'comment') AND a.id IS NOT NULL THEN
        jsonb_build_object(
          ...
          'start_page',      NULLIF(a.payload->>'start_page', '')::NUMERIC::INT,
          'end_page',        NULLIF(a.payload->>'end_page', '')::NUMERIC::INT,
          'duration_minutes',NULLIF(a.payload->>'duration_minutes', '')::NUMERIC::INT,
