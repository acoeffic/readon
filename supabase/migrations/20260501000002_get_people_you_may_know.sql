...
-- Suggestions multi-signal de profils à connecter, pondérées :
--   amis communs       × 4
--   livres communs     × 3   (status='finished' des deux côtés)
--   clubs communs      × 2
--   genre dominant     × 2   (top genre du candidat ∈ top 3 genres caller)
--
-- Pré-filtrage : un candidat doit avoir au moins une intersection sur les
-- 3 sources (amis-d-amis ∪ co-lecteurs ∪ co-membres) pour être considéré.
-- Évite un scan complet de profiles.
--
-- Retourne pour chaque candidat un tableau JSONB `reasons` avec les types
-- ayant un signal > 0, pour que l'UI affiche une explication ("3 amis en
-- commun · 5 livres en commun"). Trié par score desc.
...
BEGIN
  ...
  caller_excluded AS (
    -- Exclure les amis acceptés et les pending dans les deux sens
    SELECT CASE WHEN requester_id = v_caller THEN addressee_id
  ...
  caller_books AS (
    ...
    FROM user_books
    WHERE user_id = v_caller AND status = 'finished'
  ),
  caller_groups AS (
    SELECT group_id FROM group_members WHERE user_id = v_caller
  ),
  ...
  -- ── Pré-sélection des candidats (union des 3 sources) ───────────
  cand_via_friends AS (
    SELECT DISTINCT
  ...
  -- ── Calcul des signaux par candidat ─────────────────────────────
  signals AS (
    SELECT
      fc.uid,
      -- Amis communs
      ...
      -- Livres en commun (terminés des deux côtés)
      COALESCE((
        SELECT COUNT(*)::INTEGER
        FROM user_books ub
      ...
      -- Genre dominant : nb de top genres du caller que le candidat a aussi lu
      COALESCE((
  ...
  enriched AS (
    SELECT
      ...
      -- Mini-avatars d'amis communs (max 3)
      COALESCE((
  ...
  SELECT
    ...
    -- Reasons array : seulement les types avec un signal > 0
    (
