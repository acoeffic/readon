-- Correction de la fonction get_user_search_data
-- La colonne is_unlocked n'existe pas dans user_badges
-- Si un badge est dans user_badges, c'est qu'il est déjà débloqué
...
BEGIN
  ...
  -- Note: Si un badge est dans user_badges, c'est qu'il est déjà débloqué
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', ub.badge_id,
      'name', b.name,
...
SELECT 'Fonction get_user_search_data corrigée avec succès!' as status;
