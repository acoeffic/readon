...
CREATE INDEX IF NOT EXISTS idx_reading_groups_is_private ON reading_groups(is_private);
CREATE INDEX IF NOT EXISTS idx_reading_groups_created_at ON reading_groups(created_at DESC);
...
DROP POLICY IF EXISTS "Users can create groups" ON reading_groups;
CREATE POLICY "Users can create groups"
  ON reading_groups FOR INSERT
  WITH CHECK (auth.uid() = creator_id);

-- Policy: Only creator can update group
DROP POLICY IF EXISTS "Creators can update their groups" ON reading_groups;
...
DROP POLICY IF EXISTS "Creators can delete their groups" ON reading_groups;
...
DROP POLICY IF EXISTS "Invitees can respond to invitations" ON group_invitations;
...
DROP POLICY IF EXISTS "View group activities if member" ON group_activities;
...
DROP POLICY IF EXISTS "Members can create activities" ON group_activities;
...
DROP FUNCTION IF EXISTS get_public_groups(INT DEFAULT 20, INT DEFAULT 0) CASCADE;
...
DROP FUNCTION IF EXISTS get_group_invitations(UUID) CASCADE;
...
DROP TRIGGER IF EXISTS update_reading_groups_updated_at ON reading_groups;
...
DROP TRIGGER IF EXISTS auto_add_creator_as_admin ON reading_groups;
